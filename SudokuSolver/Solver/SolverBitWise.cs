...
//            foreach (var (x, y) in AllCoordinates(board).Where(p => board[p.x, p.y] != Cell.Empty))
//                HandleKnownDigit(x, y);
...
//                foreach (var (x, y) in AllCoordinates(board).Where(p => board[p.x, p.y] == Cell.Empty))
//                {
//                    var options = DigitOptions(optionsArr[x, y]).ToArray();
//                    if(options.Length == 0)
//                        yield break; //found conflict. no solutions
//                    if (options.Length > 1)
//                        continue;
//                    var option = options[0];
//                    board[x, y] = (Cell)option;
//                    HandleKnownDigit(x, y);
//                    changed = true;
//                } 
//            }
